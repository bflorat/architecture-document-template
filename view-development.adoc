...
//🏷{"id": "84cd4aed-36c0-4564-8354-29a7de004923", "labels": ["detail_level::overview", "constraint"]}
## Constraints
...
[TIP]
====
Contrary to the constraints which fixed the framework to which any application had to conform, the non-functional requirements were given by the project decision-makers. Schedule interviews to determine them. If some requirements are not realistic, mention this in the reference points to be decided. Avoid introducing new ergonomic “concepts” so as not to confuse users (principle of least astonishment).
====
...
====
Example: All features must be usable using the **keyboard only**.
====
...
====
Example: Validation must be **progressive and immediate** (field-level), without waiting for final submission.
====
...
====
Example: Text must remain readable when the browser is **zoomed up to 200%**, without loss of content or functionality.
====
...
====
Example: On smartphone screens, the primary navigation must collapse into a single accessible menu, and all core features must remain usable without horizontal scrolling.
====

//🏷{"id": "30cc6226-2213-4351-83aa-a4905c5d4baa", "labels": ["level::advanced", "detail_level::in-depth"]}
#### Progressive Web Apps (PWA) Requirements
...
When addressing an audience whose you do not manage browsers (such as a website on the Internet), the best option to make things intelligible and clarify the issues is to negotiate with the stakeholders of the project a minimum percentage audience supported based on https://gs.statcounter.com/[statistics]. For example: "Support 95% of browsers".

====

WARNING: Supporting old browsers (IE in particular) can generate prohibitive additional costs and security issues. In all cases, the additional costs of testing on multiple platforms should be assessed. There are good (paid) tools like Litmus or EmailOnAcid to render websites and HTML emails on a combination of OS / player type (PC / tablet / mobile) / browser very large. This type of site is essential when testing applications intended for the general public.

====
Example 1: The intranet application X must work on internally qualified browsers (see [Ref xyz])
====
====
Example 2: Application Y being an internet application targeting the widest possible audience, including terminals in developing countries. It must be tested on browsers corresponding to the 95th percentile of usage.
====
====
Example 3: Application Z is aimed at the broadest audience and with reasonably old systems and will therefore have to support: Firefox 135+, Chrome 126+, Safari 5+, Opera 20+, Edge.
====
...
====
====
Example 1: The application will be developed in Kotlin Jetpack Compose with local storage based on an H2 database synchronized with the common database by REST calls.
====
====
...
[TIP]
====
List here for each dependency the main libraries and frameworks used as well as their major version. Do not list the libraries supplied to the runtime by the application servers or the frameworks. There is no need to list every minor library: focus on the structuring dependencies.
====
...
[cols = '2s, 1,1,1,1,4a']
|===
| Type of test | Appropriate Time to invest | Manual or automated? | Type of module targeted | Target Coverage Rate | Detail
...
| Written in Playright, Cypress.js, Selenium or similar technology. They will be limited to a role of smoke tests (detection of gross problems). These tests will not be mocked but will be run against an end-to-end instantiated linking chain. To avoid unnecessary work, these tests will be done at the level of entire features. These tests will also serve as system tests since they will require a maximum of actual (non-mocked) modules.
...
* SCI: gCO₂e (grams of CO₂ equivalent) per functional unit

Potential energy-efficiency improvement areas for the project (state of the art):

* Use **modern browser profiling tools**
  (Chrome DevTools, Firefox Profiler, Safari Web Inspector), along with
  **Lighthouse** and **WebPageTest**, to analyze:
  - number of requests,
  - request size,
  - request duration,
  - CPU usage and network impact.

* For mobile applications:
  - Android: **Battery Historian**, **Android Studio Energy Profiler**;
  - iOS: **Xcode Instruments – Energy Log**;
  to identify energy-intensive phases and inefficient background activity.

* Use **specialized software sustainability tools**:
  - **Greenspector** (application-level energy analysis);
  - **Scaphandre**, **Kepler**, or **PowerAPI / Intel RAPL** for server and container energy estimation;
  - **Cloud Carbon Footprint** for cloud-based workloads (AWS, GCP, Azure).

* Optimize media assets:
  - adopt modern formats (**AVIF**, **WebP**);
  - apply lossless compression where applicable;
  - dynamically adapt image sizes to device capabilities (responsive images).

* Optimize CPU and memory usage:
  - eliminate unnecessary processing;
  - profile applications regularly;
  - for Java applications, optimize allocation patterns and configure
    **modern garbage collectors** (G1, ZGC, Shenandoah, depending on use case).

* Implement **lazy loading strategies**:
  - defer non-critical resources;
  - progressively load data based on user interaction.

* Reduce data volumes:
  - apply pagination or cursor-based APIs;
  - enforce strict server-side filtering;
  - avoid over-fetching (use batching where appropriate).

* Group compute-intensive operations into **scheduled batch jobs**
  to improve energy efficiency through workload consolidation.

====
====
Example 1: The application build process (Vite) automatically applies image
optimization through a dedicated plugin (e.g. AVIF/WebP generation), reducing
network usage and client-side energy consumption.
====
====
Example 2: Long-running endurance and robustness tests are executed on the mobile
application after each optimization cycle to assess changes in energy and battery
consumption over time.
====
====
Example 3: Performance testing campaigns include a detailed analysis of bandwidth
usage and CPU cycles, even when response-time requirements are met, in order to
identify further opportunities for sustainability-driven optimization.
====
====
Example 4: For a maximum requirement of **100 kgCO2/year** for online service A:

- the service uses **20%** of a physical server;
- the embodied carbon footprint of the server is estimated at **1.5 tCO2**
  over a 10-year lifecycle;
- resulting in **30 kgCO2/year** attributed to service A.

Using the **SCI formula**, with an average server power draw of **800 W**,
**20,000 requests per hour**, and a US electricity carbon intensity of
approximately **400 gCO2/kWh**, annual energy consumption must not exceed
**175 kWh/year**, corresponding to **0.98 mWh per request** maximum.
====
...
====
====
Example: Our PHP application will store in an HTTP session only its shopping cart, not the product references.
====
...
The configuration will be injected at runtime via environment variables provided in a `ConfigMap`. No live reloading.
====
...
[TIP]
====
What is versioned and what versioning scheme is used?
====
...
* The overall version of the project will be: `<lot>.<Sprint number>.<Deployment number>`

====

//🏷{"id": "79682de3-09b7-46f1-8354-9371295d18a8", "labels": ["level::intermediate",  "detail_level::in-depth"], "link_to":["9efde825-9508-4669-918c-7cfb0d45c21f"]}
### Concurrency
...
====
Example 1: The only encoding allowed in all modules and infrastructure components is UTF-8. The use of ISO-8859-1, CP-1252 or any other encoding is strictly prohibited. This includes the configuration of application servers (Node, Tomcat ...), sources (including `.properties` files), configuration files, databases, and files.
====
====
...
====
====
Example 2: Dates and date-times will be stored in database as epoch millis in long integer format. Dates must be stored using the ISO-8601 `YYYY-MM-DD` format without time false precision.
====

//🏷{"id": "96ec879c-3ce3-4e48-a3f9-84590c281fd4", "labels": ["detail_level::detailed"],"link_to":["0a25770c-6a02-4fa3-82cc-bf5152d3cba6"]}
### Log management
...
```java
log.info("Issue on item {} for user {}",item.getId(), user.getId());
```
====
...
[TIP]
====
Describe how you meet the i18n requirements mentioned above.
====
