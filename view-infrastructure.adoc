...
[TIP]
====
This point of view is also often called "technical point of view" and concerns the infrastructure: servers, networks, operating systems, databases, middleware, ...
...
| HI1
| We assume that by the time of the project PEM, PostgreSQL 11 will be validated internally.
| ====
...
This chapter has an educational vocation because it recalls the possible maximum availability: the final availability of the application can only be lower.
====
...
[TIP]
====
Provide the elements making it possible to estimate the average incident detection time.
====
====
...
[TIP]
====
Provide the elements making it possible to estimate the guaranteed recovery time. Note that it is important to distinguish MTTD from GTR (or MTTR in English) because it is not because a failure is detected that the skills or resources necessary for its correction are available.

Specify the hours of presence of operators during the day and the possibilities of on-call duty.

List here the intervention times of hardware, software, electricity, telecom providers ...
====
====
...
==== Levelat the service of the data center
...
[TIP]
====
PRA (Business Recovery Plan) like PCA (Business Continuity Plan) respond to a disaster risk on the IS (natural disaster, industrial accident, fire ...). An PRA allows activity to be resumed following a disaster after a certain period of restoration. It requires at least a doubling of the data center.

A PCA makes it possible to continue the critical activities of the organization (generally in a degraded mode) without notable interruption, see standard ISO 22301. This principle is reserved for very mature organizations because it requires expensive and complex technical devices (distributed filesystems and competitors for example).

The PRI (what interests the architect) is Plan de Reprise Informatique. It defines the technical means and procedures for restoring data and services within the framework of a PRA or a PCA. The PRI does not use the same technologies depending on whether it is aimed at a PRA or a PCA. For example, if we are targeting a PCA, we must plan for multi-zone active-active clusters (located in geographically distant data centers) while for a PRA, the important thing is the quality and speed of data backup / restore in the backup data center.

Note: in most large accounts, both PRA and PCA involve replication by optical link of SAN arrays to limit RPO to a minimum and ensure that all data in the datacenter is well replicated. Traditional backup / restore systems are rarely sufficient to cover this need. The difference is that in the case of a PRA, it is necessary to plan a switchover and a consequent preparation of the backup data center whereas in the case of a PCA, two (or more) data centers operate in parallel in active / active mode. nominally.
...
* For a PRA, the recovery devices (OS, data, applications) provided.
* For a PRA, give the organization's Recovery Time Objective (maximum allowable recovery time in hours) and Recovery Point Objective (maximum allowable time of lost data in hours).
* For a PCA the data replication devices (synchronous? Flow of water? How many transactions can be lost?).
...
====
Example 2: Given the very high level of security of the application, the solution should only be operated internally by sworn agents. For the same reason, cloud solutions are excluded.
====
...
====
====
Example 1: A VM should only host a single Postgresql instance
...
[TIP]
====
Contrary to the constraints which fixed the framework to which any application had to conform, the non-functional requirements are given by the project leaders (MOA in general).

Schedule interviews to collect them.
...
====
====
Example 1: All components of this application must be able to run concurrently. In particular, batch / HMI concurrency must always be possible because the batches must be able to run during the day if there is a need to catch up
====
====
...
image :: diagrams / archi-infra.svg [MIEL deployment diagram]
====
...
====
====
Example 1: the application logs of the service-honey component will be in production at INFO level with daily rotation and two-month conservation.
====
====
...
Macroscopically describe the planned procedure as well as the planned backtracking.
