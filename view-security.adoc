...
| SSH Public Key | Possession | Strong | Asymmetric key pair (public/private) for authentication. | Loss/theft of private key = compromised access. Complex management. | Server access (SSH, Git). | RFC 4251
...
| Hardware key (YubiKey) | Possession | Very strong | Physical key (USB/NFC) for strong authentication (FIDO2). | Cost (~€50). Requires compatible device. | Admin access, sensitive accounts, crypto assets. | FIDO2
...
[cols="e,e,e,e"]
|===
| Group or user | Role `delete` | Role `administration` | Role `basic data consultation`
