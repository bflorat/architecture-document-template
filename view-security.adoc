...
## Introduction
This is the security point of view. It describes all the measures implemented to prevent unauthorized use, misuse, illegitimate modification or misappropriation of the information system.
...
[TIP]
====
Authentication verifies that a user is truly who they claim to be by validating their identity using one or more proof factors.

⚠ **Do not confuse authentication with identification**, which only distinguishes one user from another without validating identity.

**Special cases:**

- **Technical accounts** (e.g., batch jobs, applications, APIs) also require authentication (e.g., service accounts with certificates or SSH keys).  
- **Privileged accounts** (e.g., `root` on servers, application administrator accounts...). These can be human or technical but have access to read/write/delete large amounts of data or perform irreversible actions (large blast radius).  
- **Initial authentication** (during registration) is often stricter than subsequent authentications.  
- **Federated authentication** allows delegation of authentication to an identity provider (SSO, OAuth2, SAML, etc.). See the following section.
...
| Possession
| Something the user **has** (e.g., token, SSH key, phone).

| Inherence
| Something the user **is** (e.g., biometrics).

| Location
| Something related to the user’s **location** (e.g., IP, GPS).

| Behavior
| Something related to the user’s **behavior** (e.g., typing rhythm).
|===

**Authentication Factors: Comparison Table**
[cols="1,1,1,2,2,1,1"]
|===
| Authentication Factor | Category | Strength | Description | Weaknesses / Risks | Recommended Use Case | Associated Standard

| Password | Knowledge | Medium | Secret string known by the user. | Vulnerable to brute force, phishing, reuse. | Basic authentication (personal accounts). | NIST SP 800-63B

| Passphrase | Knowledge | Strong | Series of easy-to-remember words (e.g., CorrectHorseBatteryStaple). | Less vulnerable to brute force but requires memorization. | Sensitive accounts (banking, email). | NIST SP 800-63B

| SSH Key | Possession | Strong | Asymmetric key pair (public/private) for authentication. | Loss/theft of private key = compromised access. Complex management. | Server access (SSH, Git). | RFC 4251

| Token-based OTP | Possession | Strong | One-time code generated by a hardware token (e.g., RSA SecurID). | Hardware dependency (loss/theft). Cost. | Sensitive logins (admin, VPN). | RFC 6238

| App-based OTP (TOTP) | Possession | Strong | Temporary code generated by an app (Google Authenticator, Authy). | Depends on mobile device. Risk if phone lost. | Online accounts (Google, AWS, banks). | RFC 6238

| Hardware key (ex: YubiKey) | Possession | Very strong | Physical key (USB/NFC) for strong authentication (FIDO2). | Cost (~€50). Requires compatible device. | Admin access, sensitive accounts, crypto assets. | FIDO2

| Digital certificate | Possession | Very strong | X.509 certificate stored on a smart card or device. | PKI management complexity. Cost. | Enterprise network/VPN access. | RFC 5280

| Biometrics | Inherence | Strong | Fingerprint, facial/voice/iris recognition. | False positives/negatives. Privacy concerns. | Mobile device unlock, physical access. | ISO/IEC 30107

| Behavioral recognition | Inherence | Medium | Typing rhythm or mouse movement analysis. | Unreliable alone. Requires learning. | Fraud detection (banking), internal access. | ISO/IEC 24741

| Business data knowledge | Knowledge | Medium | Question based on internal knowledge (e.g., “What was the 2023 revenue?”). | Hard to memorize. Can be shared. | Internal access (HR, finance). | (Internal)

| Verification link by email | Possession | Weak | Link sent by email to verify identity. | Vulnerable to phishing and interception. | Password reset. | (No specific standard)

| Push notification | Possession | Strong | Notification sent to a trusted device (e.g., Duo Security, Okta Verify). | Depends on network/mobile connectivity. | Mobile apps (banking, social networks). | (Proprietary)

| Secret question | Knowledge | Weak | Personal question (e.g., “Name of your first pet?”). | Vulnerable to guessing and leaks. | Account recovery. | (Discouraged by NIST)

| PIN code | Knowledge | Weak | Short numeric code (4–6 digits). | Limited and brute-force vulnerable. | Device/card unlock. | PCI DSS

| Geolocation | Location | Medium | IP/GPS-based location verification. | Can be bypassed by VPN/proxy. | Remote access (VPN), geo-restricted access. | (No specific standard)

| Network-based authentication | Location | Medium | Restriction to authorized IPs/networks. | Restrictive (e.g., office-only). | Internal enterprise access. | ISO/IEC 27001

| Device-based authentication | Possession | Strong | Registered device (trusted phone or computer). | Depends on device possession. | Apple/Google accounts, mobile devices. | (Proprietary)

| Continuous authentication | Behavior | Strong | Periodic re-verification (e.g., biometric every 4h). | Complex implementation. | High-security environments (banks, government). | NIST SP 800-63B
|===

**Recommendations by Use Case:**

**Basic Access** (e.g., Public Website)
[cols="1,2"]
|===
| Factor | Example

| Password
| `Tr0pF0rt!2024` (12+ characters).

| Email-based OTP
| Verification link sent by email.
|===

**Sensitive Access** (e.g., Bank Account)
[cols="1,2"]
|===
| Factor | Example

| Password
| Long passphrase (`CorrectHorseBatteryStaple`).
...
**Administrator Access** (e.g., Cloud Server)
[cols="1,2"]
|===
| Factor | Example

| SSH key + passphrase
| RSA 4096-bit key + passphrase.

| Physical token OTP (YubiKey)
| YubiKey OTP or FIDO2.

| Geolocation
| Restriction to corporate IPs.
|===

**Internal Access** (e.g., HR Data)
[cols="1,2"]
|===
| Factor | Example

| Digital certificate
| Smart card or X.509 certificate.

| Business data knowledge
| “What was the 2023 revenue?”

| Push notification
| Duo Security or Microsoft Authenticator.
|===
...
| Geolocation
| Restriction to allowed countries.

| Hardware key (YubiKey)
| YubiKey + PIN.
|===

The table below indicates the **required authentication factors** by usage context (example):

[cols="e,e,e,e,e,e,e"]
|===
|Use Case |Password meeting security policy |Known SSH public key |Token-based OTP |Biometrics |Business data knowledge |Verification link by email  
...
|Login to mobile app Y  
||||X|||  
|===

**Best Practices:**

* **MFA (Multi-Factor Authentication)** → avoid password-only authentication.  
  Require **at least a second factor** (OTP, FIDO2 key, biometrics, etc.).

* **Use proven standards** → favor well-established protocols such as  
  **FIDO2**, **WebAuthn**, **TOTP**, **SAML 2.0**, **OpenID Connect**.

* **Secure service accounts** → avoid **static passwords**.  
  Prefer **SSH keys**, **client certificates**, or **short-lived JWT tokens**.

* **Plan for revocation and renewal** → implement mechanisms to  
  **revoke and regenerate quickly** a lost, stolen, or compromised authentication factor.

* **Limit authentication attempts** → temporarily lock the account or subject  
  when the **maximum failure threshold** is reached, and **add a growing delay** between attempts  
  to reduce **brute-force attacks**.

* **Implement regular secret rotation** → periodically renew keys, certificates,  
  and tokens to reduce the potential impact of a compromise.

* **Use a modern password policy** → favor long and easy-to-remember passphrases (rather than complex ones),  
  no forced rotation, no mandatory special characters, verified against compromised password lists (*haveibeenpwned*),  
  allow copy/paste (to facilitate password manager use), and complement with **multi-factor authentication (MFA)**.  
  Reference: ANSSI, NIST SP 800-63B.
====
...
In modern information systems, non-repudiation relies almost systematically on cryptographic mechanisms.
...
[TIP]
====
An authorization (also referred to as **access right**, **entitlement**, **privilege**, or **permission**) makes it possible to **control access** to a specific application function for a user or a group of users.
...
⚠ It is recommended **not to excessively multiply** functions and roles in order to avoid a combinatorial explosion and high management costs.

**Best practices to simplify authorization management:**

- **Group** users into **groups** (e.g., `G_chef_service`).
- **Associate** a list of **functions** with a **role** (e.g., `R_Administrator`, `R_banker_level1`, `R_chef_service`).
- **Assign** roles to **users or groups** to improve factorization and maintainability.

**Classic RBAC-style authorization management model:**  

image::diagrams/roles.svg[Classic role management]

**Pseudo-users and predefined roles**:  
Remember to specify generic users such as:

- **`@anonymous`**: unauthenticated users.
- **`@authenticated`**: authenticated users.

**Authorization delegation (OAuth2, etc.):**
...
- **Is the application an authorization provider or a consumer?**
- **Which types of authorizations are involved?**
====

====
Example 1: **Unauthenticated users** will have **access to read-only privileges only**.
====

====
Example 2: The application will use a **matrix-based authorization management** approach based on an association  
**[roles] → [groups or users]**.  
Authorization details will be documented in the **FSD (Functional Specification Documents)**.
====

**Example of a role matrix:**  
[cols="e,e,e,e"]
|===
| Group or user | Role `deletion` | Role `administration` | Role `core data consultation`
...
The main authorization management models are as follows:

- **RBAC (Role-Based Access Control)**:  
  Permissions are assigned through roles.  
  A simple, readable, and widely used model, but with limited expressiveness without extensions.

- **Constraint-based RBAC (Extended RBAC)**:  
  RBAC is enhanced with conditional rules (time, location, scope).  
  This is the **most common compromise** between security and complexity.

- **ABAC (Attribute-Based Access Control)**:  
  Authorization decisions are based on the evaluation of attributes  
  (user, resource, action, environment).  
  A very flexible model, but **more complex to govern and audit**.

- **PBAC (Policy-Based Access Control)**:  
  A broader term that often builds on ABAC, allowing complex authorization rules to be defined as **policies** and evaluated by **policy engines**  
  (e.g., **OPA – Open Policy Agent**).

NOTE:  
In the mid-2020s, many organizations adopt a **hybrid access control model**, where:

- **RBAC** is used to manage **baseline roles and responsibilities**, and
- **ABAC / PBAC** is leveraged to enforce **fine-grained, contextual authorization rules**  
  (location, time, resource attributes, security posture, etc.).
- The choice of the model must remain proportional to the security stakes.  
In most information systems, **RBAC enriched with contextual constraints** provides an excellent balance between security, readability, and maintainability.


**Contextual authorizations (Extended RBAC / ABAC)**

====
In many information systems, authorizations do not depend solely on assigned roles, but also on **contextual factors**.  
These mechanisms make it possible to refine access controls based on the actual execution context.

**Examples of commonly used contextual factors:**

- **Location**:
  * geographic area (country, region),
  * source network (intranet, VPN, Internet),
  * authorized IP address ranges.
- **Time**:
  * time windows (business hours),
  * weekdays / weekends,
  * validity period of an entitlement.
- **Technical context**:
  * type of device (workstation, mobile, bastion host),
  * security level of the device (compliant device, MFA enabled),
  * access channel (API, administration interface).
- **Business or data context**:
  * resource state (draft, validated, archived),
  * functional or organizational scope (e.g., X is allowed to consult data for Asia only),
  * data classification or sensitivity.

====

====
**Examples of conditional authorizations:**

Example 1: A user with the role `R_Administrator` may delete data  
  **only from the internal network**.

Example 2: A department head may approve cases  
  **only for their assigned region**.

Example 3: Access to sensitive operations is authorized  
  **only during business hours** and with **strong authentication**.
...
For the most sensitive data, traceability can be implemented at two levels (including traceability of access to audit logs) in order to prevent abusive or excessive hierarchical monitoring.

Traceability for repository data (typically master data such as person records) requires full historization, which is considered a best practice in any case.
This involves recording both the modification date and the effective date for each change.
Audit logs must also be subject to defined retention and purging policies.
====
...
====
====
Example 2: Any VPN access must be logged.
====
====
...
List here the measures ensuring the required level of integrity.

**Examples of mechanisms associated with the required integrity level:**

- **Detectable** →  
  Access logs, integrity checks using digital fingerprints (hashes), and a posteriori detection of data alteration.

- **Controlled** →  
  Version retention, auditability of changes, and automatic correction or recomputation mechanisms when an error is detected.

- **Integrity ensured** →  
  Encryption, digital signatures, synchronous replication, immutable storage (WORM), trusted timestamping, and strong integrity proofs.

[cols="e,e,e"]
|===
| Data class | Required level | Measures

| Core business database data
| Integrity ensured
a|
* Use of the **PostgreSQL** RDBMS with a **SERIALIZABLE** transaction isolation level.
* Entities are referenced exclusively using **technical IDs** generated from PostgreSQL sequences.
* Activation of **WAL journaling** to ensure crash recovery.
* Periodic integrity checks using `pg_checksums`.

| Archived data
| Detectable
| Generation of **SHA-256** checksums for backups and systematic validation during restore operations.

| Computed data D1
| Controlled
| Storage of a **SHA-1** checksum, with automatic batch recomputation triggered in case of divergence within a maximum delay of **24 hours**.

| NoSQL silo for Big Data prior to consolidation
| Not integrity-protected
| No specific measures, **no backups**: data is temporary, non-critical, and fully recalculable.

| Application source code
| Integrity ensured
a|
* Use of **Git SCM** with native integrity controls (SHA-1 / SHA-256 hashing).
* Commit verification using **GPG signatures**.
* Strict merge strategy (**fast-forward only**).

| Tax assessment PDF
| Integrity ensured
a|
* **Digital signature** of the net amount, date, and name using **PKCS#7 (RSA, SHA-256)**.
* **Qualified timestamp** embedded in the signature (**PAdES**).
* Inclusion of the hexadecimal signature fingerprint in the **PDF footer** for subsequent verification.
|===
====
...
- The method of authentication secrets storage and verification.  
...
* **SBOM (Software Bill of Materials)**:  
  a structured inventory of software components and their dependencies, enabling:
  - **rapid identification of exposure** to newly disclosed vulnerabilities,
  - improved **software supply chain security**,
  - support for compliance and risk assessments.
====
Example 1: [SCA solution] Integration of **OWASP Dependency-Check** into the CI/CD pipeline to detect vulnerable libraries.  
====
