...
TIP: List here the possible constraints related to the computing power (CPU, GPU, TPU)
[Example]
====
...
====
Example 3: On TPU v5e, an inference model is allocated a dedicated slice of 8 chips (≈128 GB of total HBM). A slice hosts no more than one “latency-critical” model.
====
...
[Example]
====
Example 2: The backbone network of the Datacenter has a bandwidth of 40 Gbps.
====
...
[TIP]
====
List here the storage needs of each module or infrastructure component (especially databases) once the application has reached full load (volume at five years for example).
...
[cols="e,e,e,e,e,e"]
|===
| Data | Description | Unit size | Number of items at 5 years | Total size | Annual increase
...
a|
 ...
 | Communications Department, report dated 01/01/2040 | Users use the application during standard office hours
|===
====
...
If the application runs on a serverless type cloud service, the load will be absorbed dynamically, but take care to *estimate the additional cost* and to set consistent consumption limits to respect the budget while ensuring a good level of service.
====

.Example: dynamic volumetric estimation of the REST endpoint `GET Detail` of the AllMyData application
|===
| Maximum rate of users connected at the same time in annual peak | S1 x F1 x 0.2 = 8K over 1h
...
#### Response Time of GUIs Requirements
...
#### Duration of batch Requirements
